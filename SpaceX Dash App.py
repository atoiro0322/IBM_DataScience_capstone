...
launch_site_set = set(spacex_df['Launch Site'])
for launch_site in launch_site_set:
    dropdown_list.append({'label': launch_site, 'value': launch_site})
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        data = site_data['class'].value_counts().rename_axis('Outcome').reset_index(name='Count')
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    data = spacex_df[(min_payload < spacex_df['Payload Mass (kg)']) & (spacex_df['Payload Mass (kg)'] < max_payload)]
    if entered_site != 'ALL':
